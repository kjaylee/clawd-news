...
from config import (
    CLAWD_ROOT, LANCE_DB_PATH, TABLE_NAME,
...
def get_files_to_index(patterns=None):
    ...
    for pattern in patterns:
        full_pattern = os.path.join(CLAWD_ROOT, pattern)
...
def build_records(filepath):
    ...
    rel_path = os.path.relpath(filepath, CLAWD_ROOT)
...
def index_all(verbose=True):
    ...
    table = db.create_table(TABLE_NAME, all_records)
...
def index_changed(verbose=True):
    ...
    if TABLE_NAME in existing_tables:
        ...
        # LanceDB: delete by filter then add
...
def index_single(filepath, verbose=True):
    """Index a single specified file."""
    if not os.path.isabs(filepath):
        filepath = os.path.join(CLAWD_ROOT, filepath)
